    ...
    for (let [k, v] of Object.entries(updatedFields)) {
      if (k.endsWith('@deleted') && data[k] && isDefined(v)) {
        data[k] = v
          ...
          .concat(
            (data[k] ?? []).filter(
              (vvv: string) => v.filter((vv: string) => vv === vvv).length == 0,
            ),
          );
        data[k.slice(0, -8)] = data[k.slice(0, -8)].filter(
          (n: any) => n && data[k].filter((nn: any) => nn === n.id).length === 0,
        );
...
export const VisitedContainer: React.FC<
    ...
    (visitedField: string, value: VisitedFieldElementValue = true) => {
      if (typeof value === 'boolean') refVisitedFields.current[visitedField] = value;
        ...
        refVisitedFields.current[visitedField] = mergeDeep(
          refVisitedFields.current[visitedField] ?? {},
          value,
        );
      }
...
function clearErrors(errors: any, changes: any) {
  ...
  for (let [k, v] of Object.entries(changes)) {
    if (Array.isArray(v)) {
      ...
      if (isEAVFWError(errors[k])) delete errors[k];
      else clearErrors(errors[k], v);
    } else {
      delete errors[k];
    }
  }
...
export const EAVFormValidation: React.FC<
  PropsWithChildren<{ initialVisitedFields?: VisitedFieldElement }>
> = ({ children, initialVisitedFields }) => {
...
export const EAVForm = <T extends {}, TState extends EAVFormContextState<T>>({
  ...
  const actions: MutableRefObject<EAVFormContextActions<T, TState>> = useRef({
    useCollector: (collector) => {
      const [collected, setCollected] = useState<EAVCollectContext<T, TState, any>>([
        collector(state),
        actions.current,
        etag,
      ]);
    ...
    updateState: (
      cb: (props: any, ctx: any) => void,
    ): { changedProp: boolean; changedValues: any } | undefined => {
      try {
