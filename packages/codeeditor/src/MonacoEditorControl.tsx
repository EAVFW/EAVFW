...
export const MonacoEditorControl: React.VFC<MonacoEditorControlProps> = ({
  entityName,
  formName,
  fieldName,
  ...
  try {
    ...
    const [data, { onChange: onFormDataChange }] = useEAVForm<any, any, any>(
    ...
    const div = useCallback((node: HTMLDivElement) => {
      if (node !== null) {
        setHeight(
          (document.querySelector("[role='tabpanel']")?.getBoundingClientRect()?.height ?? 100) -
            55,
        );
        setWidth(
          (document.querySelector("[role='tabpanel']")?.getBoundingClientRect()?.width ?? 100) - 40,
        );
      }
    }, []);
    ...
    const handleEditorOnChange: OnChange = useDebouncer(
      (value) => {
        if (value) {
          const content = {
            ...(data[attribute.logicalName.slice(0, -2)] ?? {
          ...
          onFormDataChange((props) => {
            props[
              isLookup(attribute.type) ? attribute.logicalName.slice(0, -2) : attribute.logicalName
            ] = isLookup(attribute.type) ? content : content.data;
          });
        }
      },
      250,
