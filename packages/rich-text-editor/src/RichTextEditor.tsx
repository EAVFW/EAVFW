...
export const htmlToPlainText = (htmlText: string) =>
  htmlToContentState(htmlText).getPlainText().replaceAll('\n', ' ');
...
export const RichTextEditor: React.VFC<RichTextEditorProps> = ({
  ...
  try {
    ...
    const [editorState, setEditorState] = useState(
      loadedValue !== undefined ? htmlToEditorState(loadedValue) : undefined,
    );
    ...
    const editorStyle = useMemo(
      ...
      [Object.values(styles?.editor ?? {})],
    );

    return (
      <Editor
        wrapperClassName="rdw-editor-wrapper-buttom"
        onFocus={() => setVisisble(alwaysVisible || true)}
        onBlur={() => setVisisble(alwaysVisible || false)}
