type ArrayElement<ArrayType> = ArrayType extends readonly (infer ElementType)[] ? ElementType : any;
...
export class ODataBuilder<T> {
  ...
  public expand<T1 extends keyof T, T2 extends T[T1]>(
    prop: T1,
    expander?: (a: ODataBuilder<T2>) => ODataBuilder<T2>,
  ) {
    if (!('$expand' in this._odata)) {
      this._odata['$expand'] = {};
    }

    this._odata['$expand']![prop as string] = expander
      ? expander(new ODataBuilder<T2>())
      : new ODataBuilder<T2>();
    this.select(prop);
    return this;
  }
  public expandCollection<T1 extends KeysMatching<T, Array<any>>, T2 extends ArrayElement<T[T1]>>(
    prop: T1,
    expander: (a: ODataBuilder<T2>) => ODataBuilder<T2>,
  ) {
    if (!('$expand' in this._odata)) {
  ...
  public build(seperator: '&' | ';' = '&'): string {
    ...
    for (let k of Object.keys(this._odata) as Array<keyof typeof this._odata>) {
      ...
      switch (k) {
        ...
        case '$orderby':
          parts.push(
            `${k}=${Object.entries(v as Required<typeof this._odata>[typeof k])
        ...
        case '$expand':
          parts.push(
            `${k}=${Object.entries(v as Required<typeof this._odata>[typeof k])
              .map(([prop, propvalue]) => trimEmptyParenthesis(`${prop}(${propvalue.build(';')})`))
              .join(',')}`,
          );
          break;
        case '$select':
