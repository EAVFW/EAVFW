...
export const BoardColumn = ({
  ...
  const handleOnItemClicked = (id: string) => {
    onItemClicked
      ? onItemClicked(id)
      : () => window?.alert(`item clicked ${id} but no function is defined to handle it`);
  };

  return (
    <div className={styles.kanbanBlock} id={stateid} onDrop={onDrop} onDragOver={allowDrop}>
      <h2>
        <strong>{title}</strong>
      </h2>
      {tasks?.map((task) => (
        <BoardTask
          ...
          key={task.id}
          drag={drag}
          handleOnItemClicked={handleOnItemClicked.bind(null, task.id)}
        />
      ))}
    </div>
  );
