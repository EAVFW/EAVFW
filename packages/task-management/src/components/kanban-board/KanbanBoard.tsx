...
const KanbanBoardColumn: React.FC<KanbanBoardColumnProps> = ({
  ...
  return (
    <div className={styles['kanban-block']} id={stateid} onDrop={onDrop} onDragOver={allowDrop}>
      ...
      {tasks?.map((task) => (
        <div key={task.id} id={task.id} draggable="true" onDragStart={drag}>
...
export const KanbanBoard: React.FC = () => {
  ...
  const boardTasks = queryEntitySWR(app.getEntityFromKey('Board Task'), {
    $filter: `boardid eq ${currentRecordId}`,
    $select: 'id,task',
  ...
  const drop = useCallback((ev: DragEvent<HTMLDivElement>) => {
    ...
    fetch(
      `${baseUrl}/entities/${app.getEntityFromKey('Task').collectionSchemaName}/records/${taskid}`,
      {
        method: 'PATCH',
        credentials: 'include',
        body: JSON.stringify({
      ...
      <div className={styles['kanban-board']}>
        {boardColumns?.data?.items?.map((column) => (
          <KanbanBoardColumn
            ...
            tasks={boardTasks?.data?.items
              ?.filter((t) => t.task.stateid === column.taskstate.id)
              .map((c) => c.task)}
        ...
        <div className={styles['create-new-task-block']} id="create-new-task-block">
          ...
          <span className={styles['form-row']}>
            <label className={styles['form-row-label']} htmlFor="task-name">
              Task
            </label>
            <input
              className={styles['form-row-input']}
              type="text"
              name="task-name"
              id="task-name"
            />
          ...
          <span className={styles['form-row-buttons']}>
            <button className={styles['edit-button']} id="edit-button" onClick={editTask}>
              Edit
            </button>
            <button className={styles['save-button']} id="save-button" onClick={saveTask}>
              Save
            </button>
            <button className={styles['cancel-button']} id="cancel-button" onClick={createTask}>
              Cancel
            </button>
          </span>
        </div>
