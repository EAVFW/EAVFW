...
export const EAVApp = ({ children, baseUrl, ...props }: PropsWithChildren<Test>) => {
  ...
  const [state, dispatcher] = useReducer(EAVAppReducer, props, (arg: typeof props) => ({
    ...
    model:
      'model' in arg
        ? arg.model
        : new ModelDrivenApp(throwIfNull(arg.manifest, 'Manifest or model must be given')),
  }));

  return (
    <EAVClientProvider baseUrl={baseUrl}>
      <AppContext.Provider
        value={[state, { toggleNav: () => dispatcher({ action: 'toggleNav' }) }]}
      >
        {children}
      </AppContext.Provider>
    </EAVClientProvider>
