...
export class ModelDrivenApp {
  ...
  getEntity(entityName: string) {
    if (!entityName) throw new Error('entityName not provided');
  ...
  getAttributes(entityName: string) {
    ...
    return {
      ...((targetEntity.TPT && this.getEntity(targetEntity.TPT).attributes) ?? {}),
  ...
  isMatchingForm(
    form: FormColumnDefinition | undefined,
    tabName: string,
    columnName: string,
    sectionName: string,
  ) {
  ...
  getEntityKey(logicalName: string) {
    return Object.entries(this._data.entityMap).filter(
      ([entityKey, entityLogicalName]) => entityLogicalName === logicalName,
    )[0][0];
  }
  ...
  getExpandQueryParam(
    ...
    expandall = false,
    includeHierachi = false,
    isReferenceLoopup?: EntityDefinition,
  ) {
    ...
    let expand = Object.values(attributes)
      ...
      .filter(
        (a) =>
          typeof isReferenceLoopup === 'undefined' ||
          a.type.referenceType !== this.getEntityKey(isReferenceLoopup.logicalName),
      )
      .map(
        (a) =>
          `${getNavigationProperty(a)}($select=id,${Object.values(this.getAttributes(this.getEntityFromKey(a.type.referenceType).logicalName)).filter((c) => c.isPrimaryField)[0].logicalName})`,
      )
      .join(',');
    return expand;
  }
  getSelectQueryParamForExpand(entityDefinition: EntityDefinition) {
    ...
    let expand = Object.values(attributes)
      .filter(isAttributeLookup)
      .map((a) => `${getNavigationProperty(a)},${getNavigationProperty(a)}id`)
      .join(',');
    return expand;
  }
      ...
      for (const entity of Object.values(this._data.entities)) {
        try {
          for (const attribute of Object.values(entity.attributes).filter(isAttributeLookup)) {
            ...
            if (
              referenceTypes
                .map((referenceType) => this._data.entityMap[referenceType])
                .some((n) => n === entityName || (tpt && this._data.entityMap[tpt] === n)) &&
              ...
              for (const formKey of Object.keys(attribute.type.forms)) {
                ...
                if (form.type === 'Main') {
                  if (
                    (formKey === formName || formName === form.name) &&
                    this.isMatchingForm(form, tabName, columnName, sectionName)
                  ) {
                    const viewName = form.view ?? Object.keys(entity?.views ?? {})[0];

                    const logicalname = attribute.type.inline
                      ? Object.entries(entity.attributes)
                          .filter(isAttributeLookupEntry)
                          .filter(
                            (x) => x[1].type.referenceType === this.getEntityKey(entityName),
                          )[0][1].logicalName
                    ...
                    references.push({
                      ...
                      polylookup: attribute.type?.split
                        ? 'split'
  ...
  getWizardsTriggeredByNew(appname: string, area: string, entityname: string) {
    ...
    let wizards = Object.entries(entity?.wizards ?? {}).filter(([key, wizard]) =>
      Object.values(wizard.triggers ?? {}).some((x) => x.ribbon && x.ribbon == 'NEW'),
    );
  ...
  getWizardsTriggeredByRibbons(appname: string, area: string, entityname: string) {
    ...
    let wizards = Object.entries(entity?.wizards ?? {})
      .filter(([key, wizard]) =>
        Object.values(wizard.triggers ?? {}).some((x) => x.ribbon && typeof x.ribbon === 'object'),
      )
      .map(
        ([key, wizard]) =>
          [
            key,
            wizard,
            Object.entries(wizard.triggers ?? {}).filter(
              ([triggerkey, x]) => x.ribbon && typeof x.ribbon === 'object',
            ) as [string, WizardTrigger][],
