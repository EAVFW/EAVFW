...
const PagingContext = createContext({
  ...
  firstItemNumber: 0,
  lastItemNumber: undefined as number | undefined,
...
export const PagingProvider: React.FC<PropsWithChildren<PagingProviderProps>> = ({
  children,
  initialPageSize = 100,
  enabled = true,
}) => {
  ...
  return (
    <PagingContext.Provider
      value={{
        ...
        fetchQuery,
        setFetchQuery,
        firstItemNumber: (fetchQuery?.$skip ?? 0) + 1,
        lastItemNumber: Math.min(totalRecords ?? 0, (fetchQuery?.$skip ?? 0) + pageSize),
        totalRecords,
        currentPage,
        pageSize,
        moveNext,
