...
export class ItemToCardResolver {
  public static convertItemsToCardObjects(
    ...
    for (let i: number = 0; i < items.length; i++) {
      ...
      Object.entries(columns).forEach((column) => {
        ...
        if (correspondingViewColumn && correspondingViewColumn?.visible !== false) {
          ...
          if (isChoice(column[1].type)) {
            ...
            const option = Object.entries(column[1].type.options!).find(
      ...
      if (!title || !subTitle) {
        const stringKeys = Object.keys(item).filter(
      ...
      const itemOtherActions = otherActions
        .filter((oa) => filterOtherActions(oa, item))
        .map((otherAction) => {
  ...
  //     }
  // }
  public static resolveOtherActions(
    items: IRecord[],
    app: ModelDrivenApp,
    buttons: ICommandBarItemProps[],
    selectedView: string,
...
function filterOtherActions(oa: OtherAction, item: IRecord): unknown {
  ...
  if (typeof oa.visibleOnCard === 'string') {
    switch (oa.visibleOnCard) {
      case '@equals(record().status,50)':
        return item.status === 50;
      case '@equals(record().status,30)':
        return item.status === 30;
      default:
        throw new Error('Not implemented, ' + oa.visibleOnCard);
    }
  }
