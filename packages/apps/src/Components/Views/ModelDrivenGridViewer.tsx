    ...
    return (
      <Link
        ...
        href={recordRouteGenerator({
          ...
          entityName:
            item[attribute.logicalName.slice(0, -2)]?.["$type"] ??
            type.foreignKey?.principalTable!,
        })}
      >
        <a>
          {
    ...
    return (
      <LookupControlRender
        onChange={(data: any) => {
          ...
          console.log("LookupControlRender change", [
            item,
            index,
            column,
            data,
            ...
            <ModelDrivenList
              ...
              onRenderItemColumn={(item, index, column) => (
                <ConditionRenderComponent
                  ...
                  attribute={attributes[column?.key!]}
                  items={items}
                  locale={locale}
                />
              )}
...
const ModelDrivenGridViewerContext =
  createContext<ModelDrivenGridViewerContextProps>({
    onRenderPrimaryField: ({ recordRouteGenerator, item, column }) => (
      <Link legacyBehavior={true} href={recordRouteGenerator(item)}>
        <a>{item[column?.fieldName!] ?? "<ingen navn>"}</a>
      </Link>
    ),
  });
