...
import { Stack, IDropdownOption, IPivotProps } from '@fluentui/react';
...
export function ModelDrivenBodyViewer({
  ...
  const views = Object.fromEntries(
    Object.entries(entity.views ?? {}).filter(([viewKey, view]) => filterRoles(view?.roles, user)),
  );
  ...
  const _onChangeView = (
    ...
    router.push(
      `/apps/${router.query.appname}/areas/${router.query.area}/entities/${router.query.entityName}/views/${option?.key}/`,
    );
  };
  ...
  return (
    <PagingProvider
      initialPageSize={
        typeof view?.paging === 'object' ? (view.paging.pageSize ?? undefined) : undefined
      }
      enabled={
        !(
          view?.paging === false ||
          (typeof view?.paging === 'object' && view?.paging?.enabled === false)
        )
      }
    >
      <Stack verticalFill className={styles.section}>
        ...
        <Stack.Item grow style={{ height: 'calc(100% - 32px)' }}>
          <RibbonHost ribbon={ribboninfo}>{BodyViewElement}</RibbonHost>
        </Stack.Item>
      </Stack>
    </PagingProvider>
  );
