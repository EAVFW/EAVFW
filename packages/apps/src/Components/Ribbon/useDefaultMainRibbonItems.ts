...
const useWizardRibbonButtons = () => {
  ...
  const wizards = useMemo(
    () =>
      app.getWizardsTriggeredByRibbons(
        appInfo.currentAppName,
        appInfo.currentAreaName,
  ...
  useEffect(() => {
    ...
    for (let [key, wizard, triggers] of wizards) {
      for (let [triggerKey, trigger] of triggers) {
        if (trigger.visibleForForms === false && appInfo.currentRecordId) continue;
  ...
  useEffect(() => {
    ...
    if (ribbonInfo.new?.visible !== false) {
      addButton({
        ...
        data: { order: 0 },
        onClick: (e, i) => {
          const wizards = app.getWizardsTriggeredByNew(
            appInfo.currentAppName,
            appInfo.currentAreaName,
            appInfo.currentEntityName,
          );
      ...
      addButton({
        ...
        onClick: (e, i) => {
          setTimeout(async () => {
            let tasks = selection
              .getSelection()
              .map((i) => deleteRecordSWR(app.getEntity(i.entityName!), i.id!));
...
export const useDefaultMainRibbonItems = (
  ribbonInfo: RibbonViewInfo = {},
  pushRoute: (url: URL) => void,
  withSave = true,
) => {
