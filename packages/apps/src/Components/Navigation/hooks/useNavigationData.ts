...
function filterEntry(user: any) {
  ...
  return ([key, entry]: [string, ModelDrivenSitemapEntry]) => {
    ...
    return (
      entry.roles.allowed?.filter((r) => user.role.filter((rr: string) => r === rr).length > 0)
        ?.length ?? 0 > 0
    );
  };
}
...
export function useNavigationData(sitemap: ModelDrivenSitemap) {
  ...
  return useMemo(() => {
    try {
      const areas = Object.keys(sitemap.areas)
        .filter((area) => {
          if (!user) return true;
          ...
          for (let g of Object.keys(sitemap.areas[area])) {
            ...
            for (let e of Object.keys(group)) {
              ...
              if (roles) {
                ...
                if (
                  roles?.allowed?.filter(
                    (role) => user.role.filter((r: string) => role === r).length > 0,
                  )?.length ??
                  0 > 0
                ) {
                  return true;
      ...
      const result = {
        ...
        groups: Object.entries(sitemap.areas[(router.query.area as string) ?? areas[0]?.key])
          .map(([groupKey, groupEntry]) => {
            const entries = Object.entries(groupEntry)
              .filter(filterEntry(user))
