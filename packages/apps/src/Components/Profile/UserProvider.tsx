...
function getProfile(entityKey?: string) {
  ...
  const { record: userInfo } = getRecordSWR(
    entityKey ? app.getEntityFromKey(entityKey).collectionSchemaName : '',
    data?.sub!,
    '',
    entityKey && data ? true : false,
  );
...
> = ({
  ...
  if (authorize) {
    if (!record || record.isAuthenticated === false) return <Loader />;
  }
