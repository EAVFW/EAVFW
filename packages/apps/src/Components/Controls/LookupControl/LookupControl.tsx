...
const _styles: IStyleFunction<IDropdownStyleProps, IDropdownStyles> = (props) => ({});
...
export const LookupCoreControl: React.FC<LookupCoreControlProps> = ({
  ...
  const initialOptions = useMemo(
    () =>
      typeof selectedValue === 'object'
        ? [{ key: selectedValue.id ?? DUMMY_DATA_KEY, text: selectedValue[primaryField] }]
        : [],
    [selectedValue],
  );
  ...
  const query = useMemo(
    () => ({ $select: `id,${primaryField}`, $top: 10, ...returnQueryFilter(searchfilter, filter) }),
    [filter, searchfilter],
  );
    ...
    () =>
      (
        ...
        remoteSelectedValue?.id
          ? [{ key: remoteSelectedValue.id, text: remoteSelectedValue[primaryField] }]
          : [],
      ),
  ...
  const [selectedKey, setSelectedKey] = useState<string | null>(
    value ?? (typeof selectedValue === 'object' ? (selectedValue.id ?? DUMMY_DATA_KEY) : undefined),
  );
  ...
  const _onFormRenderDataChange = useCallback(
    (data: any, localctx: EAVFormOnChangeCallbackContext) => {
...
export function LookupControl<T>({
  ...
  const [{ selectedValue, value, formvalues }, { onChange: eavOnChange }] = useEAVForm((state) => {
    ...
    if (isSplit && selectedValue)
      selectedValue =
        selectedValue[
          Object.entries(app.getEntity(selectedValue['$type']).attributes)
      ...
      value:
        isPolyLookup(attribute.type) && attribute.type.split
          ? (attribute.type.referenceTypes
              ...
              .flat()[0] ?? state.formValues[logicalName]?.split(':')[1])
  ...
  if (isPolyLookup(attribute.type)) {
    ...
    const defaultEntity = () => {
      if (type.split && selectedValue) return app.getEntityKey(selectedValue['$type']);
