...
export const WizardTabs: React.FC<{
    ...
    return Object.entries(tabs)
      ...
      .map(([key, tab]) => (
        <Tab
          ...
          disabled={
            isTransitioning ||
            Object.keys(tabs).indexOf(key) > Object.keys(tabs).indexOf(selectedTab)
          }
          icon={
            Object.keys(tabs).indexOf(key) < Object.keys(tabs).indexOf(selectedTab) ? (
          ...
          value={key}
        >
          {tab?.locale?.[app.locale]?.title ?? tab.title ?? key}
        </Tab>
      ));
  };
  ...
  return (
    <div className={mergeClasses(stack.root, stack.horizontal, className)}>
      ...
      <WizardTab
        tabName={selectedTab}
        className={mergeClasses(stack.itemGrow)}
        columns={tabmodel.columns}
        controlName={tabmodel.control}
      />
    </div>
  );
};
