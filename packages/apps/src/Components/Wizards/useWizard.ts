    ...
    context.with(state.otelContext!, () => {
      setTimeout(async () => {
        ...
        if (transition) {
          ...
          let rsp = await fetch(
            `${process.env.NEXT_PUBLIC_API_BASE_URL}/workflows/${transition.workflow}/runs`,
            {
              method: 'POST',
          ...
          while (!completed) {
            let statusRsp = await fetch(
              `${process.env.NEXT_PUBLIC_API_BASE_URL}/workflows/${transition.workflow}/runs/${id}/status`,
              {
                //headers: {
                //    'traceparent': `00-${spanContext.traceId}-${spanContext.spanId}-0${spanContext.traceFlags}`
                //},
          ...
          let stateRsp = await fetch(
            `${process.env.NEXT_PUBLIC_API_BASE_URL}/workflows/${transition.workflow}/runs/${id}`,
            {
              //headers: {
              //    'traceparent': `00-${spanContext.traceId}-${spanContext.spanId}-0${spanContext.traceFlags}`
              //},
