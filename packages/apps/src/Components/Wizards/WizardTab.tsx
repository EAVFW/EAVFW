...
export const WizardTab: React.FC<{
  ...
  if (!columns || Object.keys(columns).length === 0) {
    if (controlName && controlName in Controls) {
      ...
      return (
        <div
          className={mergeClasses(stack.verticalFill, stack.horizontal)} /* tokens={StackTokens}*/
        >
          <Component />
        </div>
      );
  ...
  const ui = (
    ...
    >
      {isTransitioning && (
        <div className={styles.root} style={{ margin: '0px' }}>
          <Spinner className={styles.spinner} size="huge" label="working..." />
        </div>
      )}
      {Object.keys(columns).map((columnName, idx) => (
        <div
          key={tabName + columnName}
          className={mergeClasses(columnName, stack.item, stack.itemGrow)}
        >
