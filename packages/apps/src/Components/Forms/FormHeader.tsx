...
export const FormHeader = ({
  ...
  const primaryField = useMemo(
    () => Object.values(app.getAttributes(entityName)).find((a) => a.isPrimaryField)!,
    [entityName],
  );
  ...
  const DisableTabsOnNewFeatureFlag =
    window?.__env?.['DisableTabsOnNewFeatureFlag'] !== 'true' &&
    typeof currentRecordId === 'undefined';
  return (
    <>
      {form?.type !== 'QuickCreate' && (
        <Stack.Item className={styles.section} styles={{ root: { marginLeft: 15, paddingTop: 8 } }}>
          <h2>{primaryFieldValue}</h2>
          <Stack horizontal style={{ alignItems: 'center' }}>
            <h3 style={{ height: '28px' }}>
              {entity.locale?.[locale]?.displayName ?? entity.displayName}
            </h3>
            {hasMoreForms && (
          ...
          <TabList
            ...
            onTabSelect={(e, data) => onTabChange(data.value as string)}
          >
            {tabs
              ...
              .map((tabName) => {
                ...
                return (
                  <Tab
                    ...
                    value={tabName}
                    key={tabName}
                    disabled={DisableTabsOnNewFeatureFlag}
                    title={DisableTabsOnNewFeatureFlag ? 'Enabled on save' : tabName}
                  >
