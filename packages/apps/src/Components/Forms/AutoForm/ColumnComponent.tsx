...
const ColumnComponent = <T extends { id?: string; [key: string]: any }>(
  ...
  try {
    ...
    useChangeDetector(
      `ColumnComponent: Tab: ${tabName} Column: ${columnName} form`,
      form,
      renderId,
    );
    useChangeDetector(
      `ColumnComponent: Tab: ${tabName} Column: ${columnName} sections`,
      sections,
      renderId,
    );
    useChangeDetector(
      `ColumnComponent: Tab: ${tabName} Column: ${columnName} tabName`,
      tabName,
      renderId,
    );
    useChangeDetector(
      `ColumnComponent: Tab: ${tabName} Column: ${columnName} columnName`,
      columnName,
      renderId,
    );
    useChangeDetector(
      `ColumnComponent: Tab: ${tabName} Column: ${columnName} entity`,
      entity,
      renderId,
    );
    useChangeDetector(
      `ColumnComponent: Tab: ${tabName} Column: ${columnName} formName`,
      formName,
      renderId,
    );
    useChangeDetector(
      `ColumnComponent: Tab: ${tabName} Column: ${columnName} formData`,
      formData,
      renderId,
    );
    useChangeDetector(
      `ColumnComponent: Tab: ${tabName} Column: ${columnName} onFormDataChange`,
      onFormDataChange,
      renderId,
    );
    useChangeDetector(
      `ColumnComponent: Tab: ${tabName} Column: ${columnName} locale`,
      locale,
      renderId,
    );
    useChangeDetector(
      `ColumnComponent: Tab: ${tabName} Column: ${columnName} factory`,
      factory,
      renderId,
    );
      ...
      <Stack verticalFill className={`form-column`}>
        {Object.keys(sections).map((sectionName, idx) => (
          <Stack.Item
            grow
            key={sectionName}
            className={`section ${sectionName} ${mergeClasses(styles.section, styles.flex)}`}
          >
            <SectionComponent<T>
              form={form}
              tabName={tabName}
