...
const iconWarningButtonStylesFunc = (theme: ITheme) =>
  ({
    root: { marginBottom: -3 },
    icon: { color: theme.palette.yellowDark },
...
export const EAVFWLabel = ({
  ...
  const iconWarningButtonStyles = useMemo(
    () => iconWarningButtonStylesFunc(_theme ?? theme),
    [_theme],
  );
  ...
  return (
    <div className={requiredStyles.label}>
      {_label}
      {required ? <span className={requiredStyles.required}>*</span> : null}
      {description && (
        <IconButton
          id={iconButtonId}
          ...
          <div className={contentStyles.body}>
            <Stack tokens={stackTokens} horizontalAlign="start" styles={labelCalloutStackStyles}>
              {warnings.map((w, i) => (
                <span
                  key={w.logicalName + i}
                  dangerouslySetInnerHTML={{ __html: w.warning }}
                ></span>
              ))}
            </Stack>
          </div>
