...
export function FormLayout(props: PageLayoutProps) {
  try {
    ...
    const [selection, setSelection] = useState<Selection<Partial<IRecord> & IObjectWithKey>>(
      new Selection<Partial<IRecord> & IObjectWithKey>({
        onSelectionChanged: () => {
    ...
    return (
      <ModelDrivenGridViewerSelectedContext.Provider
        value={{ setSelection, selection: selection!, selectionDetails }}
      >
