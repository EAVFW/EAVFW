import {
  DashboardDefinition,
  EntityDefinition,
  isSingleSiteMapDefinition,
  ManifestDefinition,
  PrimitiveType,
  SiteMapDefinition,
  EntityLocaleDefinition,
  MultipleSiteMapDefinitions,
  ManifestAppsDefinition,
} from '@eavfw/manifest';
...
function sortObj<T>(
  areas: { [key: string]: T },
  sort: (a: T, ai: number, b: T, bi: number) => number,
  childMapper: (a: T) => T,
) {
    ...
    (a, ai, b, bi) => {
      const aa = Math.max(
        ...
        Math.max(
          -1000 + ai,
          ...Object.values(a)
            .map((x) => Object.values(x).map((xx) => xx.order))
            .flat(),
        ),
      );
      const ab = Math.min(
...
function isEntityDefinition(item: any): item is EntityDefinition {
  ...
  return (
    ...
    'collectionSchemaName' in item &&
    typeof item.collectionSchemaName === 'string' &&
    'attributes' in item &&
...
function getTitle(
  ...
  const title =
    sitemap.title ??
    sitemap.locale?.[locale].pluralName ??
    sitemap.locale?.[locale]?.displayName ??
    (item.locale?.[locale] as EntityLocaleDefinition)?.pluralName;
  if (
    title === undefined &&
    item.locale?.['1030'] &&
    (item.locale['1030'] as EntityLocaleDefinition).pluralName !== 'undefined'
  ) {
    return (
      (item.locale['1030'] as EntityLocaleDefinition).pluralName ??
      item.displayName ??
      item.pluralName
    );
  }
...
function processSitemap(
  ...
  if (typeof sitemaps === 'object') {
    ...
    const test = sitemaps;
    for (const sitemapKey of Object.keys(sitemaps).sort(
      (sitemapKeyA, sitemapKeyB) =>
        (test[sitemapKeyA].order ?? Infinity) - (test[sitemapKeyB].order ?? Infinity),
    )) {
      ...
      const groupTitle =
        app?.sitemap?.groups?.[sitemap.group]?.locale?.[locale]?.title ??
        app?.sitemap?.groups?.[sitemap.group]?.title ??
        sitemap.group;
      ...
      let title =
        selectedLocale && 'pluralName' in selectedLocale
          ? selectedLocale.pluralName
          : (selectedLocale?.displayName ?? item.pluralName ?? item.displayName);
...
function processItems(
  ...
  for (const key of Object.keys(items).sort(
    (a, b) => getOrder(items[a].sitemap) - getOrder(items[b].sitemap),
  )) {
...
export function generateAppContext(
  ...
  const entities = Object.assign(
    {},
    ...Object.values(manifest.entities).map((o) => ({ [o.logicalName]: o })),
  );
  const dashboards = manifest.dashboards
    ? Object.assign(
        {},
        ...Object.entries(manifest.dashboards).map(([key, value]) => ({
